          ...
          {serviceCards.map((service, index) => {
            ...
            return (
              <StaggerItem key={service.title}>
                <MotionArticle
                  index={index}
                  className="group relative flex min-h-100 flex-col items-center overflow-hidden rounded-[1.6rem] border border-emerald-100 bg-white px-7 py-10 text-center shadow-xl shadow-green-950/10 transition-shadow duration-300 hover:shadow-2xl hover:shadow-green-950/15"
                >
                  <div className="absolute inset-x-0 top-0 h-1 bg-linear-to-r from-[#47a51f] via-[#79c24f] to-[#064118] opacity-0 transition duration-300 group-hover:opacity-100" />
                  <div className="absolute -right-12 -top-12 size-32 rounded-full bg-lime-100/60 transition duration-500 group-hover:scale-125" />

                  <div className="relative grid size-24 place-items-center rounded-[1.4rem] bg-emerald-50 text-[#064118] ring-1 ring-emerald-100 transition duration-300 group-hover:bg-[#064118] group-hover:text-[#eff7e8]">
                    <Icon className="size-12" strokeWidth={1.65} aria-hidden="true" />
                    {index === 0 ? (
                        <Waves className="absolute -bottom-3 -right-3 size-8 rounded-full bg-white p-1.5 text-[#064118] shadow-md shadow-green-950/10" aria-hidden="true" />
                      ) : index === 1 ? (
                        <Leaf className="absolute -bottom-3 -right-3 size-8 rounded-full bg-white p-1.5 text-[#064118] shadow-md shadow-green-950/10" aria-hidden="true" />
                      ) : (
                        <ClipboardCheck className="absolute -bottom-3 -right-3 size-8 rounded-full bg-white p-1.5 text-[#064118] shadow-md shadow-green-950/10" aria-hidden="true" />
                      )}
                  </div>

                  <h3 className="mt-8 min-h-[5.8rem] text-[clamp(1.25rem,1.55vw,1.55rem)] font-black leading-tight tracking-[-0.03em] text-[#064118]">
                    {service.title}
                  </h3>

                  <div className="mt-2 h-1 w-16 rounded-full bg-[#47a51f] transition duration-300 group-hover:w-24" />
